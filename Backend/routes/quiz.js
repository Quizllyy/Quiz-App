...
router.get("/:quizId", async (req, res) => {
    try {
        ...
        // 🛑 Validate if quizId is a valid 24-character MongoDB ObjectId
        if (!mongoose.Types.ObjectId.isValid(quizId)) {
            return res.status(400).json({ message: "Invalid quiz ID format. Must be a 24-character hex string." });
        }
